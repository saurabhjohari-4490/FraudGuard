# Setup Guide - FraudGuard Platform

Installation, prerequisites, and step-by-step setup for the FraudGuard fraud detection platform. For system design and scoring details, see [README.md](./README.md).
...
│  └──────────┘ └──────────┘                                  │
├──────────────────────────────────────────────────────────────┤
│  Decision Engine: 0-30=approve | 31-60=verify | 61-100=escalate │
│                   (no auto-block — analyst only)               │
└──────────────┬───────────────────────────┬──────────────────┘
               │                           │
    ┌──────────▼──────────┐     ┌──────────▼──────────┐
...
```bash
git clone git@github.com:razorpay/fraudguard.git
cd fraudguard
```
...
```
NAME               STATUS                  PORTS
fraudguard-postgres-1     running (healthy)       0.0.0.0:5433->5432/tcp
fraudguard-redis-1        running (healthy)       0.0.0.0:6380->6379/tcp
fraudguard-fraud-api-1    running (healthy)       0.0.0.0:8001->8000/tcp
fraudguard-dashboard-1    running                 0.0.0.0:3001->3000/tcp
```
...
```json
{
  ...
  "explanation": "ESCALATED - Risk score 72.5/100. Unusual spending pattern from unrecognized device.",
...
- Filter by: All | Critical | Escalated | Verify | Blocked
...
Final score = weighted sum of module scores (with critical boost + multi-module amplifier). Decision thresholds:
- **0-30** → Approve (transaction passes, user profile updated)
- **31-60** → Verify (step-up authentication, alert queued)
- **61-100** → Escalate (manual analyst review required; no auto-block)
